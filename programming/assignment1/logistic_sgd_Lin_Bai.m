function [ theta ] = logistic_sgd_Lin_Bai( data, labels )
	...
	M = size(data,1); % values in each sample
	theta = zeros(M,1);
	theta_new = zeros(M,1);
	diff_J = zeros(M,1);
...
% the function g(theta_T*x)=1/(1+theta_T*x)
...
% update theta = theta - alpha*diff(J(theta))
	...
	for k = 1:N
%		for j = 1:M
%			diff_J = (1/(1+transpose(theta)*data(:,k))-labels(j))*data(j,k);
%			theta_new(j)=theta(j)-alpha*diff_J;
%		end
		diff_J = (1/(1+transpose(theta)*data(:,k))-labels)*data(:,k);
